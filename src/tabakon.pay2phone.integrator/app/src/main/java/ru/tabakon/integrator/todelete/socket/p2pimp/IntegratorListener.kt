package ru.tabakon.integrator.todelete.socket.p2pimp

import org.java_websocket.handshake.ServerHandshake
import ru.tabakon.integrator.log
import ru.tabakon.integrator.todelete.socket.IWebSocketCallback
import ru.tabakon.integrator.todelete.socket.TabakonWebSocketClient
import java.net.URI
...
interface IIntegratorListener {
    ...
    fun setOnMessage(onMessage : (message: String?) -> Unit)
}
...
class IntegratorListenerBuilder{
    ...
    fun build() : IIntegratorListener{
        val target = IntegratorListener(_uri, _onOpen, _onClose, _onSending)
